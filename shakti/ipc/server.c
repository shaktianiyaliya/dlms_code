...
int main ( )
{
	...
	int sock ;
	struct sockaddr_in server; // In this structure have all server address  
	int mysock ;
	char buff[1024] ;
	int rval ;
...
// the process to block until the client connects to the server.  it returns a new descriptor and all communication should be carried out using 	the new file descriptor
	do{
		mysock = accept ( sock ,( struct sockaddr * ) 0, 0 ) ; // New socket create 
